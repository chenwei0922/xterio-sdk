/**
 * 是否禁用 hCaptcha 验证
 */
export const disableCaptchaVerify = true

export const errCodeMap = {
  11001: 'Wrong email or password.',
  11004: 'Too many attempts to login.',
  11102: 'User already registered.',
  11103: 'Invalid password.',
  11111: 'Invalid verification code.',
  11112: 'Verification code expired.',
  11113: 'User does not exist.',
  11401: 'Invalid or expired signature.',
  11402:
    'The wallet has been paired up with another account: {{uuid}}. You can use the wallet to sign in to that account.',
  11403: 'The account has reached the maximum number of linked wallets.',
  unknown: 'Unknown error, please try again later.'
}
